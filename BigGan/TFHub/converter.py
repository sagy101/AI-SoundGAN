...
from BigGan.TFHub import biggan_v1 as biggan_for_conversion
...
def dump_tfhub_to_hdf5(module_path, hdf5_path, redownload=False):
  ...
  # CHANEGED module to load
  hub.load(module_path)
...
class TFHub2Pytorch(object):
  ...
  def load_generator(self):
    ...
    #YAHAV changed to:
    self.load_linear('linear', 'linear', bias=False)
...
def generate_sample(G, z, z_dim, batch_size, orig_filename, parallel=False, category=513, samples=1, mode="bigGAN"):
  
  G.eval()
  G.to(DEVICE)
  with torch.no_grad():
    #Sagy changed: random z to input z and mode
    if mode == "bigGAN":
      z = torch.randn(samples, G.dim_z).to(DEVICE)
    y = category*torch.ones(size=(samples,), dtype=torch.int64, device=DEVICE)
    if parallel:
  ...
  for i in range(images.size()[0]):
    if(images.size()[0] == 1):
      filename = orig_filename + '.jpg'
    else:
      filename=orig_filename + str(i)+ '.jpg'
    save_image(images[i], filename, scale_each=True, normalize=True)

def generate_batch(G, z, parallel=False, category=513):
  category = torch.from_numpy(category).to(DEVICE)
  with torch.set_grad_enabled(True):
    if parallel:
      images = nn.parallel.data_parallel(G, (z, G.shared(category)))
    else:
      images = G(z, G.shared(category))
  return images
